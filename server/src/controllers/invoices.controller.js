const invoicesService = require("../services/invoices.service");
const paymentsService = require("../services/payments.service");

class InvoicesController {
  async create(req, res, next) {
    try {
      const created = await invoicesService.create(req.body);
      return res.status(201).json(created);
    } catch (err) {
      next(err);
    }
  }

  async getAll(req, res, next) {
    try {
      const invoices = await invoicesService.getAll();
      return res.status(200).json(invoices);
    } catch (err) {
      next(err);
    }
  }

  async getById(req, res, next) {
    try {
      const { id } = req.params;
      const invoice = await invoicesService.getById(id);
      return res.status(200).json(invoice);
    } catch (err) {
      next(err);
    }
  }

  async confirm(req, res, next) {
    try {
      const { id } = req.params;
      const confirmed = await invoicesService.confirm(id);
      return res.status(200).json(confirmed);
    } catch (err) {
      next(err);
    }
  }

  async pay(req, res, next) {
    try {
      const { id } = req.params;
      const payment = await paymentsService.payInvoice(id, req.body);
      return res.status(201).json(payment);
    } catch (err) {
      next(err);
    }
  }

  async delete(req, res, next) {
    try {
      const { id } = req.params;
      const deleted = await invoicesService.delete(id);
      return res.status(200).json(deleted);
    } catch (err) {
      next(err);
    }
  }
}

module.exports = new InvoicesController();
