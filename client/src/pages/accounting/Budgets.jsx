...
export default function Budgets() {
  ...
  // view: 'list' | 'form' | 'view' | 'revise'
  const [view, setView] = useState('list');
  const [selectedBudget, setSelectedBudget] = useState(null);
  // reviseData holds the editable state for the revision form
  ...
  // Open revise form pre-filled with original lines
  const openReviseForm = (budget) => {
    setReviseData({
      originalId: budget.id,
      name: budget.name + ' Revised',
      period_start: budget.period_start ? new Date(budget.period_start).toISOString().split('T')[0] : '',
      period_end: budget.period_end ? new Date(budget.period_end).toISOString().split('T')[0] : '',
      responsible_contact_id: budget.responsible_contact_id || budget.responsible_contact?.id || '',
      lines: (budget.budget_lines || []).map(l => ({
        analytic_account_id: l.analytic_account_id,
        type: l.type || 'EXPENSE',
        committed_amount: l.committed_amount || 0,
        allowed_amount: l.allowed_amount || 0,
      }))
    });
    setView('revise');
  };
  ...
  const handleSaveRevision = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        lines: reviseData.lines
      };
      const res = await fetch(`${base}/${reviseData.originalId}/revise`, {
  ...
  const handleAction = async (action, id) => {
    ...
    try {
      ...
      if (action === 'confirm') {
        ...
        if (!window.confirm('Archive/cancel this budget?')) { setIsSaving(false); return; }
  ...
  // ─── Revise Form (editable, pre-filled) ─────────────────────────────────────
  ...
  if (view === 'revise' && reviseData) {
    ...
    const updateRevLine = (idx, field, val) => {
      ...
      setReviseData({ ...rd, lines });
    };
    const addRevLine = () => setReviseData({ ...rd, lines: [...rd.lines, { analytic_account_id: '', type: 'EXPENSE', committed_amount: '', allowed_amount: '' }] });
    const removeRevLine = (idx) => setReviseData({ ...rd, lines: rd.lines.filter((_, i) => i !== idx) });
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button className="primary-btn" onClick={handleSaveRevision} disabled={isSaving}
              style={{ backgroundColor: '#2563EB', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <RefreshCw size={16} /> {isSaving ? 'Saving...' : 'Confirm Revision'}
            </button>
            <button className="secondary-btn" onClick={() => { setReviseData(null); setView('view'); }}>Cancel</button>
          </div>
        </div>

        <div style={{ background: '#DBEAFE', border: '1.5px solid #93C5FD', borderRadius: '10px', padding: '12px 16px', marginBottom: '20px', fontSize: '0.875rem', color: '#1E40AF' }}>
          <strong>Creating a Revision</strong> — The original budget will be archived as REVISED. The new revised budget will be CONFIRMED and linked back to the original.
        </div>

        <div style={{ background: '#FFFFFF', border: '2px solid #1E293B', borderRadius: '16px', padding: '24px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px' }}>
            <div className="form-field">
              <label style={{ fontWeight: '600', fontSize: '0.85rem' }}>Revised Budget Name</label>
              <input value={rd.name} onChange={e => setReviseData({ ...rd, name: e.target.value })} />
            </div>
            <div className="form-field">
              <label style={{ fontWeight: '600', fontSize: '0.85rem' }}>Responsible</label>
              <select value={rd.responsible_contact_id}
                onChange={e => setReviseData({ ...rd, responsible_contact_id: e.target.value })}>
                <option value="">-- Select Contact --</option>
                {contacts.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
            <div className="form-field">
              <label style={{ fontWeight: '600', fontSize: '0.85rem' }}>Start Date</label>
              <input type="date" value={rd.period_start}
                onChange={e => setReviseData({ ...rd, period_start: e.target.value })} />
            </div>
            <div className="form-field">
              <label style={{ fontWeight: '600', fontSize: '0.85rem' }}>End Date</label>
              <input type="date" value={rd.period_end}
                onChange={e => setReviseData({ ...rd, period_end: e.target.value })} />
            </div>
          </div>

          <h3 style={{ fontSize: '1rem', fontWeight: '700', marginBottom: '12px' }}>Budget Lines — Edit Revised Amounts</h3>
          <table className="valora-table" style={{ width: '100%', marginBottom: '16px' }}>
            <thead>
              <tr>
                <th>Analytic Account</th>
            ...
            <tbody>
              {rd.lines.map((line, idx) => (
                <tr key={idx}>
                  <td>
                    <select value={line.analytic_account_id}
                      onChange={e => updateRevLine(idx, 'analytic_account_id', e.target.value)}
                      style={{ width: '100%', padding: '6px', borderRadius: '6px', border: '1px solid #CBD5E1' }}>
                      <option value="">-- Select Analytic --</option>
                      {analyticAccounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
                    </select>
                  ...
                  <td>
                    <select value={line.type} onChange={e => updateRevLine(idx, 'type', e.target.value)}
                      style={{ width: '100%', padding: '6px', borderRadius: '6px', border: '1px solid #CBD5E1' }}>
                      <option value="EXPENSE">Expense</option>
                      <option value="INCOME">Income</option>
                    </select>
                  ...
                  <td>
                    <input type="number" value={line.committed_amount}
                      onChange={e => updateRevLine(idx, 'committed_amount', e.target.value)}
                      style={{ width: '100%', padding: '6px', borderRadius: '6px', border: '1px solid #CBD5E1', textAlign: 'right' }} />
                  </td>
                  <td>
                    <input type="number" value={line.allowed_amount}
                      onChange={e => updateRevLine(idx, 'allowed_amount', e.target.value)}
                      style={{ width: '100%', padding: '6px', borderRadius: '6px', border: '1px solid #CBD5E1', textAlign: 'right' }} />
                  </td>
                  <td style={{ textAlign: 'center' }}>
                    <button type="button" onClick={() => removeRevLine(idx)}
                      style={{ background: 'none', border: 'none', color: '#EF4444', cursor: 'pointer' }}>
                      <Trash2 size={16} />
                    </button>
                  </td>
          ...
          <button type="button" className="secondary-btn" onClick={addRevLine}
            style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
            <Plus size={16} /> Add Line
          </button>
        </div>
